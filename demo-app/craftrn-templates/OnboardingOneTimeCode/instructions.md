# Onboarding One Time Code Template - AI Customization Guide
...
- **Digit-by-Digit Input**: Individual input fields for each OTP digit
- **Auto-Focus Management**: Automatic advancement between fields
- **Paste Support**: Handle clipboard OTP pasting
- **Auto-Submit**: Automatic verification when complete

### 2. Timer and Resend Pattern

- **Countdown Timer**: Visual countdown for resend availability
- **Resend Logic**: Handle OTP regeneration and delivery
- **Rate Limiting**: Prevent abuse of resend functionality
- **Multiple Delivery Options**: SMS, email, voice call options

### 3. Verification Flow

- **Real-time Validation**: Immediate feedback on code entry
- **Error Handling**: Clear messaging for invalid codes
- **Success States**: Confirmation of successful verification
- **Retry Mechanisms**: Handle failed verification attempts
...
export const useResendOTP = () => {
  return useMutation({
    mutationFn: async ({ phoneNumber, deliveryMethod }: {
      phoneNumber: string;
